...
int main()
{
	...
	for(int i = 0 ; i < num ; ++i)
	{
		...
		if(type == 1)
		{
			((Lines *)arr[i])->setPoints();
		}

		else if(type == 2)
		{
			((Quads *)arr[i])->setPoints();
		}

		else
		{
			((Polygs *)arr[i])->setPoints();
		}
	}
	...
	while(opType != 4)
	{
		...
		else if(opType == 1)
		{
			for(int i = 0 ; i < num ; ++i)
			{
				...
				if(type == 1)
				{
					((Lines *)arr[i])->DrawShape();
				}
		
				else if(type == 2)
				{
					((Quads *)arr[i])->DrawShape();
				}
		
				else
				{
					((Polygs *)arr[i])->DrawShape();
				}
			}
		}
		...
		else if(opType == 2)
		{
			...
			for(int i = 0 ; i < num ; ++i)
			{
				...
				if(type == 1)
				{
					((Lines *)arr[i])->move(xT, yT);
				}
		
				else if(type == 2)
				{
					((Quads *)arr[i])->move(xT, yT);
				}
		
				else
				{
					((Polygs *)arr[i])->move(xT, yT);
				}
			}
		}
		...
		else if(opType == 3)
		{
			...
			if(type == 1)
			{
				((Lines *)arr[num])->EraseShape();
			}
		
			else if(type == 2)
			{
				((Quads *)arr[num])->EraseShape();
			}
		
			else
			{
				((Polygs *)arr[num])->EraseShape();
			}
